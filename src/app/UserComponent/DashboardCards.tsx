...
export default function DashboardCards({ upcomingTask, latestTask, pendingCount, finishedCount }: Props) {
    return (
        ...
        mt-5 mb-5 px-3 space-x-5">
            <div className="bg-primaryVividOrange w-1/3 p-3 rounded-2xl">
                ...
                <p className="text-3xl font-bold">
                    {upcomingTask ? `Task #${upcomingTask.id}` : "No Task"}
                </p>                
                <p className="text-sm">
                    {upcomingTask
            ...
            <div className="bg-nuanceDarkPastelBlue w-1/3 p-3 rounded-2xl">
                ...
                <p className="text-3xl font-bold">Task #{latestTask?.id ?? "No task found"}</p>
                <p className="text-sm"> Added on: {latestTask?.createdAt ? new Date(latestTask.createdAt).toLocaleDateString() : "N/A"}</p>
            </div>
            <div className="w-1/3 rounded-2xl flex flex-col space-y-2">
                <div className="bg-nuanceDarkPastelBlue flex-1 px-3 py-1">
                    ...
                    <p className="text-center">{pendingCount}</p>
                </div>
                <div className="bg-nuanceDarkPastelBlue flex-1 px-3 py-1">
                    <p className="text-sm italic font-semibold">FINISHED DEADLINE</p>
                    <p className="text-center">{finishedCount}</p>
                </div>
            </div>
        </div>
    )
